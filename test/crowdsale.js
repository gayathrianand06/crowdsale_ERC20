...
var solutoken = artifacts.require("solutoken");
...
contract('crowdsale', function(accounts) {
    ...
    it('one ETH should buy 0.01 Hashnode Tokens in PreICO', function(done){
        HashnodeCrowdsale.deployed().then(async function(instance) {
            ...
            const solutoken = soluoken.at(tokenAddress);
    ...
    it('one ETH should buy 0.02 Hashnode Tokens in ICO', function(done) {
        HashnodeCrowdsale.deployed().then(async function(instance) {
            const data = await instance.sendTransaction({ from: accounts[2], value: web3.toWei(1.5, "ether")});
            const tokenAddress = await instance.token.call();
            const solutoken = solutoken.at(tokenAddress);
            const tokenAmount = await solutoken.balanceOf(accounts[2]);
            assert.equal(tokenAmount.toNumber(), 5000000000000000000, 'The sender didn\'t receive the tokens as per ICO rate');
            done();
       });
    });
